...
class SRResNet(nn.Module):
  def __init__(self, in_channels, out_channels, nker=64, norm="bnorm", learning_type="plain", nblk=16):
    ...
    ps1 = []
    ps1 += [nn.Conv2d(in_channels=nker, out_channels=4 * nker, kernel_size=3, stride=1, padding=1)] # out_channels에 4곱한 이유 : 채널사이즈가 64->256으로 바뀌었기 때문
    ...
    ps2 = []
    ps2 += [nn.Conv2d(in_channels=nker, out_channels=4 * nker, kernel_size=3, stride=1, padding=1)] # out_channels에 4곱한 이유 : 채널사이즈가 64->256으로 바뀌었기 때문
